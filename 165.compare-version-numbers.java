...
class Solution {
    ...
    public int compareVersion(String version1, String version2) {
        ...
        int start = 0;
        while(start < v1.length && start < v2.length) {
            int val1 = Integer.parseInt(v1[start]);
            int val2 = Integer.parseInt(v2[start]);
            if(val1 < val2) return -1;
            if(val1 > val2) return 1;
            start++;
        }
        while(start < v1.length) {
            int val = Integer.parseInt(v1[start]);
            if(val > 0)
                return 1;
            start++;
        }
        while(start < v2.length) {
            int val = Integer.parseInt(v2[start]);
            if(val > 0)
                return -1;
            start++;
        }
